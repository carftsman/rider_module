    ...
    try {
      ...
      if (
        ...
        return res.status(400).json({
          success: false,
          message:
            "name, cityId, scenarioType and vehicleType are required"
        });
      }
      ...
      if (
        ...
        return res.status(400).json({
          success: false,
          message:
            "Some pincodes are invalid or inactive"
        });
      }
      ...
      if (duplicateConfig) {
        return res.status(400).json({
          success: false,
          message:
            "Another active pincode config already exists",
          data: {
            existingConfigId:
              duplicateConfig.id
          }
      ...
      return res.status(200).json({
        success: true,
        message:
          "Pincode payout config updated successfully",
        data: updatedConfig
      });
  ...
  try {
    ...
    if (duplicateConfig) {
      return res.status(400).json({
        success: false,
        message:
          "Another active city config already exists",
        data: {
          existingConfigId:
            duplicateConfig.id
        }
    ...
    return res.status(200).json({
      success: true,
      message:
        "City payout config updated successfully",
      data: updatedConfig
    });
